...
export default class Search extends React.Component {
  ...
  handleInputChange(e) {
    if (e.target.value.length > 2 && !(e.target.value === this.suggestionsArray[0])) {
      this.suggestionsArray = this.trie.suggest(e.target.value);
    }
    console.log('suggestions', this.suggestionsArray)
    this.setState({
  ...
  handleSubmit(e) {
    ...
    console.log('woot')
    if (this.suggestionsArray[0].includes(inputVal)) {
